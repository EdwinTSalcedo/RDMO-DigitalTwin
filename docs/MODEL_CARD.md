...
The implementation includes a single-pass assertion. During training and inference, the code verifies that the YOLO backbone is entered exactly once per image batch.
...
The current reported model was trained for 30 epochs.
...
| Validation final 5-class mAP50 | 0.7098 |
...
```text
experiments/NEXT_EXPERIMENTS.md
```
