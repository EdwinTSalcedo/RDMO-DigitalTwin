...
def find_repo_root(start_dir):
    ...
    while True:
        candidate = os.path.join(current, "models", "model_finetuned.pt")
...
REPO_ROOT = os.environ.get("RDMO_REPO_ROOT", find_repo_root(BASE_DIR))
DEFAULT_MODEL_PATH = os.path.join(REPO_ROOT, "models", "model_finetuned.pt")
MODEL_PATH = os.environ.get("RDMO_MODEL_PATH", DEFAULT_MODEL_PATH)
DETECTIONS_DIR = os.environ.get(
    "RDMO_DETECTIONS_DIR",
    os.path.join(BASE_DIR, "Detecciones_model_pt")
)
...
app = FastAPI(
    title="RDMO UAV Recovery Inference Server",
    description="API de inferencia usando models/model_finetuned.pt por defecto",
    version="1.1",
    lifespan=lifespan
)
